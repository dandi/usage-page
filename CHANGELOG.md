...
- Ordered the per-Dandiset table with its scaled metrics first and the raw totals they derive from after, renamed those totals to "Total Bytes", "Total Views", "Total Downloads" and "Total Requests", and widened that table's window so all ten columns are visible at once on a wide screen. Table columns gained an optional `default_sort` flag, used to keep "Total Bytes" the column the table is sorted by on load now that it is no longer the leftmost metric. ([#241](https://github.com/dandi/usage-page/pull/241))
