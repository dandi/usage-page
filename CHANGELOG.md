...
- Added an "Ignore testing datasets" checkbox to the per-Dandiset section's gear-wheel settings panel, which leaves out Dandisets `000027`, `000126` and `000717` — whose usage is dominated by automated testing of the archive, and which otherwise dominate the scaled metrics. The setting is remembered in the URL (`ignore_testing=true`), applies to the table (including its download) rather than the plot, and is shown only for the archive-wide selection, the only one whose rows are Dandisets. ([#241](https://github.com/dandi/usage-page/pull/241))
