...
- Added an "Ignore testing dataset" checkbox to the per-Dandiset table view, which leaves out Dandisets `000027`, `000126` and `000717` — whose usage is dominated by automated testing of the archive, and which otherwise dominate the scaled metrics. The setting is remembered in the URL (`ignore_testing=true`), applies to the table (including its download) rather than the plot, and is shown only where it applies: the table view of the archive-wide selection. ([#241](https://github.com/dandi/usage-page/pull/241))
