...
- Ordered the per-Dandiset table with its scaled metrics first and the raw totals they derive from after, each scaled group sitting beside its own denominator: the three per-asset rates with "Total Assets", then "Bytes / Size", then the usage totals, closing on the directly comparable "Total Bytes" and "Total Size" pair. Renamed the raw totals to "Total Bytes", "Total Views", "Total Downloads" and "Total Requests", and let that table use the full page width (instead of the 1100px the other tables are capped at) so all of its columns are visible at once on a wide screen. Table columns gained an optional `default_sort` flag, used to keep "Total Bytes" the column the table is sorted by on load now that it is no longer the leftmost metric. ([#241](https://github.com/dandi/usage-page/pull/241))
