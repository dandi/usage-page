...
- Added a "Metric" dropdown to each of the first two plot sections, choosing which metric the plot is drawn in rather than always bytes. The over-time plot offers "Bytes", "Views" and "Downloads"; the per-Dandiset histogram (the archive-wide selection) additionally offers the three scaled metrics — "Views / Asset", "Downloads / Asset" and "Bytes / Size" — which are hidden for any other selection, since only Dandisets have a known asset count and stored size to divide by. Both selections are remembered in the URL (`ot_metric`, `hist_metric`), and both selectors are shown only in plot view, the table views listing every metric as their own columns already. Each plot's title names the metric being plotted ("Views per day", "Views / Asset per Dandiset"), and its y-axis drops the byte suffix and byte decade labels for the metrics that are not bytes. ([#243](https://github.com/dandi/usage-page/pull/243))
