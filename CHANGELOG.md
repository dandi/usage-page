...
- Added a "Metric" dropdown to each of the first two plot sections, choosing which metric the plot is drawn in rather than always bytes. The over-time plot offers "Bytes", "Views" and "Downloads"; the per-Dandiset histogram (the archive-wide selection) additionally offers the three scaled metrics — "Views / Asset", "Downloads / Asset" and "Bytes / Size" — which are hidden for any other selection, since only Dandisets have a known asset count and stored size to divide by. Both selections are remembered in the URL (`ot_metric`, `hist_metric`), and both selectors are shown only in plot view, the table views listing every metric as their own columns already. The histogram selector lists its metrics in the column order of the table beside it — "Views / Asset", "Downloads / Asset", "Views", "Downloads", "Bytes / Size", "Bytes" for the per-Dandiset table, and "Bytes", "Views", "Downloads" for the per-asset one — so the two read the same way round, and each selection opens on the first metric of its own list. The per-Dandiset plot therefore now opens on "Views / Asset", which compares Dandisets of very different sizes on equal terms, rather than on raw bytes; the per-asset plot still opens on "Bytes". Each plot's title names the metric being plotted ("Views per day", "Views per Asset per Dandiset" — a title spells a scaled metric's ratio out, where the dropdown and table heading keep the shorter "Views / Asset"; "Bytes / Size" is titled "Bytes transferred relative to total size of Dandiset", that ratio not surviving being spelled out), and its y-axis drops the byte suffix and byte decade labels for the metrics that are not bytes. ([#243](https://github.com/dandi/usage-page/pull/243))
