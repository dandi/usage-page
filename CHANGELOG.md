...
- Ordered the metric columns of every table view as "Views", "Downloads", "Requests" (after the leading "Usage" column). ([#240](https://github.com/dandi/usage-page/pull/240))
- Named the byte unit in the over-time plot's title instead of the generic "Usage", so it now reads e.g. "TB per day" (or "TiB per day" with the binary prefix selected) and "Total TB to date" in cumulative mode. The unit is derived from the largest plotted value, accounting for stacking in the grouped modes. Table views keep the generic "Usage per day" wording, since their rows are each formatted to their own unit. ([#240](https://github.com/dandi/usage-page/pull/240))
- Anchored the default sort of every table view to its first numeric column (the "Usage" metric) instead of its last column, so appending further metric columns no longer changes which column tables are sorted by on load. ([#240](https://github.com/dandi/usage-page/pull/240))
