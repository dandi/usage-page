...
- Opened both maps farther south and cut their world at the Bering Strait, so the region map's "Oceania" label is no longer cut off at its right-hand edge. ([#272](https://github.com/dandi/usage-page/pull/272))
- Carried the region map's black labels and the removal of its "America" label into PNG and SVG downloads, which had drawn CARTO's own grey labels. ([#273](https://github.com/dandi/usage-page/pull/273))
...
- Filled in the map's colorbar in the Chromatic snapshots, where it had been drawn as an empty outline: its gradient is referenced from an inline style, which the browser records as an absolute URL into a page the archive is no longer served from. ([#251](https://github.com/dandi/usage-page/pull/251))
- Served a flat stand-in basemap to the Chromatic snapshot run in place of the live CARTO one, taking the last live request out of a suite that mocks everything else. ([#251](https://github.com/dandi/usage-page/pull/251))
