...
// The ISO 3166 table generated by src/scripts/build-iso-region-codes.mjs, which
// turns the codes of by_region.tsv into names and GADM boundaries.
let ISO_REGION_CODES: IsoRegionCodes | null = null;
...
// Load the generated ISO 3166 table (cached after first load).  A failure here
// is not fatal to the geography section: the labels fall back to the raw codes
// and the choropleth to matching subdivisions by name, so the request is made
// once and its outcome shared by everything that reads a region key.
let ISO_REGION_CODES_PROMISE: Promise<void> | null = null;
function load_iso_region_codes(): Promise<void> {
    ISO_REGION_CODES_PROMISE ??= fetch("iso_region_codes.json")
        .then(r => { if (!r.ok) throw new Error("Failed to fetch ISO region codes"); return r.json(); })
        .then(data => { ISO_REGION_CODES = data as IsoRegionCodes; })
        .catch(error => { console.error("Error loading ISO region codes:", error); });
    return ISO_REGION_CODES_PROMISE;
}

// Split a region key from by_region.tsv against the loaded ISO 3166 table
function parse_region(region: string): ParsedRegion {
    return parse_region_key(region, ISO_REGION_CODES);
}

// Load TopoJSON and name aliases (cached after first load)
function load_choropleth_data(): Promise<void[]> {
    const promises: Promise<void>[] = [load_iso_region_codes()];
    if (!GEOJSON_DATA) {
        promises.push(
...
// Build lookups from "iso2/normalized name" to feature index for fast matching.
// The names are normalized here as well as on the way in, so that a boundary
// GADM spells with an accent is still found by a summary that spells it without.
function build_geojson_lookup(): { lookup: Record<string, number>; country_lookup: Record<string, Array<[string, number]>> } {
    ...
    GEOJSON_DATA!.features.forEach((feature, idx) => {
        ...
        const name_norm = normalize_region_name(feature.properties.name_norm);
...
function match_region_to_feature(region: string, lookup: Record<string, number>, country_lookup: Record<string, Array<[string, number]>>): number {
    const parsed = parse_region(region);
    if (parsed.kind !== "subdivision") return -1;
    let country_code = parsed.country_code as string;
    let subdivision_name = parsed.subdivision_name as string;

    // An ISO 3166-2 code whose boundary the generated table already knows needs
    // no matching by name: the two datasets were reconciled by coordinate when
    // the table was built, including where they divide a country differently.
    if (parsed.gadm_name && !(country_code in COUNTRY_REMAPPING)) {
        const generated_key = `${country_code}/${normalize_region_name(parsed.gadm_name)}`;
        if (generated_key in lookup) return lookup[generated_key];
    }
    ...
    if (NAME_ALIASES && NAME_ALIASES[country_code]) {
        ...
        if (aliased) {
            const alias_key = `${country_code}/${normalize_region_name(aliased)}`;
...
function load_top_regions_table(by_region_summary_tsv_url: string): Promise<void> {
    return fetch(by_region_summary_tsv_url)
        ...
        .then((text) => {
            ...
            // Filter out cloud regions, sort by bytes descending.  The region
            // is shown by name rather than by the ISO codes it arrives as, and
            // rows that resolve to the same place are summed: while the archive
            // is being reprocessed one place can arrive both under an ISO 3166-2
            // code and under the subdivision name it used to be reported by, and
            // the choropleth already counts the two as one region.
            type RegionTotals = { region: string; bytes: number; requests: number; downloads: number; views: number };
            const totals_by_region = new Map<string, RegionTotals>();
            data.forEach((row) => {
                const parsed = parse_region(row[0]);
                if (parsed.kind === "cloud") return;
                const totals = totals_by_region.get(parsed.label) ??
                    { region: parsed.label, bytes: 0, requests: 0, downloads: 0, views: 0 };
                totals.bytes += parseInt(row[1], 10) || 0;
                totals.requests += parseInt(row[2] || "0", 10) || 0;
                totals.downloads += parseInt(row[3] || "0", 10) || 0;
                totals.views += parseInt(row[4] || "0", 10) || 0;
                totals_by_region.set(parsed.label, totals);
            });
            const regions = Array.from(totals_by_region.values());
...
function load_geographic_heatmap(dandiset_id: string): Promise<void | void[] | [void, void]> {
    ...
    // Every view here labels regions from the ISO 3166 table, so all of them
    // wait on it rather than each racing the same request.
    const topRegionsPromise = load_iso_region_codes()
        .then(() => load_top_regions_table(by_region_summary_tsv_url));
        ...
        .then(([, text]) => {
            ...
            data.forEach((row) => {
                ...
                if (coordinates) {
                    ...
                    hover_texts.push(
                        `${parse_region(region).label}<br>${human_readable_bytes_sent}` +
    ...
    .then(([, text]) => {
        ...
        feature_bytes.forEach((bytes, idx) => {
            if (bytes > 0) {
                ...
                const country_name = ISO_REGION_CODES?.countries[iso2] ?? iso2;
                hover_texts.push(
