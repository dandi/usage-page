...
// The region table generated by src/scripts/build-region-info.mjs, which turns
// the ISO 3166 codes of by_region.tsv into names and GADM boundaries.
let REGION_INFO: RegionInfo | null = null;
...
// Load the generated region table (cached after first load).  A failure here is
// not fatal to the geography section: the labels fall back to the raw codes and
// the choropleth to matching subdivisions by name, so the request is made once
// and its outcome shared by everything that reads a region key.
let REGION_INFO_PROMISE: Promise<void> | null = null;
function load_region_info(): Promise<void> {
    REGION_INFO_PROMISE ??= fetch("region_info.json")
        .then(r => { if (!r.ok) throw new Error("Failed to fetch region info"); return r.json(); })
        .then(data => { REGION_INFO = data as RegionInfo; })
        .catch(error => { console.error("Error loading region info:", error); });
    return REGION_INFO_PROMISE;
}

// Split a region key from by_region.tsv against the loaded region table
function parse_region(region: string): ParsedRegion {
    return parse_region_key(region, REGION_INFO);
}

// Load TopoJSON and name aliases (cached after first load)
function load_choropleth_data(): Promise<void[]> {
    const promises: Promise<void>[] = [load_region_info()];
    if (!GEOJSON_DATA) {
        promises.push(
...
function load_geographic_heatmap(dandiset_id: string): Promise<void | void[] | [void, void]> {
    ...
    // Every view here labels regions from the region table, so all of them wait
    // on it rather than each racing the same request.
    const topRegionsPromise = load_region_info()
